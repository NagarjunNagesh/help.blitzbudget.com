...
(function scopeWrapper ($) {
  const searchArticleDD = document.getElementById('searchArticleDD')
  ...
  const faq = []
  const categoryInformation = window.categoryInfo[window.currentLanguage]
  const gettingstartedtitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.gettingstarted : 'Getting Started'
  const budgettitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.budget : 'Budget'
  const transactionstitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.transactions : 'Transactions'
  const goalstitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.goals : 'Goals'
  const financialaccountstitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.financialaccounts : 'Financial Accounts'
  const miscellaneoustitle = isNotEmpty(window.translationData) ? window.translationData.dynamic.miscellaneous : 'Miscellaneous'
  const articlesdescription = isNotEmpty(window.translationData) ? window.translationData.dynamic.articles : ' articles'
  for (let i = 0, len = categoryInformation.length; i < len; i++) {
    ...
    switch (categoryInfoItem.categoryName) {
      ...
      case transactionstitle:
        ...
        break
      case goalstitle:
        document.getElementById('goalsCount').innerText = isEmpty(subCategoryArr)
          ? 0 + articlesdescription
  ...
  function retrieveAppropriateInformation (anchorHref) {
    ...
    const urlPath = lastElement(anchorHref.split('help.blitzbudget.com'))
    // Check if language is in the URL
    const languageInUrl = '/' + window.currentLanguage + '/'
  ...
  function populateCategoryNav () {
    const categoryInfo = window.categoryInfo[window.currentLanguage]
  ...
  function makeSubCategoryActive (urlToMatch) {
    ...
    for (let i = 0, len = categoryItems.length; i < len; i++) {
      ...
      const anchorItem = categoryItem.lastElementChild
  ...
  document.getElementById('askUsDirectly').addEventListener('click', function (e) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        // send Email
        const email = document.getElementById('emailIdAUD').value
        const message = document.getElementById('askUsDirectlyText').value
        const subject = isNotEmpty(window.translationData) ? window.translationData.dynamic.customersupport : 'Customer Support: Requesting More Information 72 hours'
  ...
  $(document).on('keyup', '#emailIdAUD', function (e) {
    ...
    const askUsDirectlyText = document.getElementById('askUsDirectlyText')
    const emailEnt = this.value
  ...
  $(document).on('keyup', '#askUsDirectlyText', function (e) {
    ...
    const emailEnt = document.getElementById('emailIdAUD').value
