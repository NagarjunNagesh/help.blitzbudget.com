<template>
  <div class="wrapper blog-post">
    <div class="page-header page-header-small rellax-header">
      <div class="page-header-image" style="background-image: url('img/bg38.jpg')"></div>
      <div class="content-center">
        <div class="row">
          <div class="col-md-8 ml-auto mr-auto text-center">
            <h2 class="title">Add Category</h2>
            <h4>Help Center</h4>
          </div>
        </div>
      </div>
    </div>
    <div class="section">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <div class="button-container">
              <nuxt-link to="#article" class="btn btn-success btn-round btn-lg">
                <i class="now-ui-icons text_align-left"></i> Read Article
              </nuxt-link>
            </div>
          </div>
        </div>
      </div>
      <div class="section" id="article">
        <div class="container">
          <div class="row">
            <div class="col-md-8 ml-auto mr-auto">
              <h3 class="title">Add Category</h3>
              <br>
              <br>
              <img src="img/gifs/category/add-category.gif" alt="Budget">
              <br>
              <br>
              Categorizing the transaction is the first step towards organizing your financial behaviour. To properly
              see what we are spending and where we are spending what we are spending.
              <br>
              <br>
              A category just has two simple fields that could be added. <strong>Category Type</strong>
              and the name of the category.
              <br>
              <br>
              <nuxt-link to="/categories">#Category</nuxt-link>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  name: "blog-posts",
  head: {
    title: 'Add Category',
  },
  layout: "default",
}
</script>
<style>
</style>